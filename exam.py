...
def programs_with_process_id(logs: list) -> list:
    ...
    for log in logs:
        ...
        log_program_with_process_id = f"{log_program}"

        if (
            log_program_with_process_id not in process_id_program
            and log_process_id != -1
        ):
            process_id_program.append(log_program_with_process_id)
